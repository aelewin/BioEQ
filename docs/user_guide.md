...
**No existing content** — pull the real, current `Imports:` list straight from `DESCRIPTION`
rather than retyping one by hand (it drifts). Also mention `Rscript launch_app.R` and the
`docs/INSTALLATION_GUIDE.md` beginner path — both exist and are current (confirmed in `README.md`)
but were never cross-referenced from this guide.
